...
def find_distance(end_position, heightmap):
    visited = set()
    distances = [[None for _ in row] for row in heightmap]
    distances[end_position[0]][end_position[1]] = 0
    ...
    for i in range(len(heightmap)):
        for j in range(len(heightmap[i])):
            if heightmap[i][j] == 0:
                if distances[i][j] is not None and (shortest_route is None or distances[i][j] < shortest_route):
                    shortest_route = distances[i][j]
                
    return shortest_route
    


heightmap, _, end_position = parse_input('input')
end_distance = find_distance(end_position, heightmap)
